    ...
    <div className="group relative">
      ...
      {showDelete && onDelete && (
        <div className="opacity-0 flex items-center justify-center group-hover:opacity-100 transition-opacity absolute right-2 top-1/2 -translate-y-1/2 pointer-events-auto">
          <DeleteChannelDialog channel={channel} onDelete={onDelete} />
        </div>
      )}
    </div>
  )
...
export function ChannelList() {
  ...
  const { deleteChannel, canDeleteChannel, createChannel } = useChannelManagement(userData?.id || '')
    ...
    return (
      <div className="px-2">
        <div className="flex items-center px-2 py-2">
          <h2 className="text-sm font-semibold text-zinc-500">Loading channels...</h2>
        </div>
      </div>
    )
  }
