  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="truncate text-sm text-zinc-700">{attachment.file_name}</div>
        <div className="text-xs text-zinc-500">{formatFileSize(attachment.file_size)}</div>
      </div>
      <Button
        variant="ghost"
        size="icon"
  ...
  return (
    <div className="flex flex-wrap gap-2 mt-1">
      {attachments.map((attachment) => (
        <AttachmentContent key={attachment.id} attachment={attachment} />
      ))}
    </div>
  )
}
